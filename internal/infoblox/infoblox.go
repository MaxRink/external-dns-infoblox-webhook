...
// isNotFoundError reports whether err is, or wraps, an Infoblox "object not
// found" answer. errors.As is required because the error is wrapped with %w on
// its way up from the paging client.
...
type StartupConfig struct {
	Host         string `name:"infoblox-host" env:"INFOBLOX_HOST" required:"" default:"localhost"`
	Port         int    `name:"infoblox-port" env:"INFOBLOX_PORT" required:"" default:"443"`
	Username     string `name:"infoblox-wapi-user" env:"INFOBLOX_WAPI_USER" required:""`
	Password     string `name:"infoblox-wapi-password" env:"INFOBLOX_WAPI_PASSWORD" required:""`
	Version      string `name:"infoblox-version" env:"INFOBLOX_VERSION" required:""`
	...
	UseTTL       bool   `name:"infoblox-use-ttl" env:"INFOBLOX_USE_TTL" default:"true"`
	ExtAttrsJSON string `name:"infoblox-extensible-attributes-json" env:"INFOBLOX_EXTENSIBLE_ATTRIBUTES_JSON" default:"{}"`
	FQDNRegEx    string
	NameRegEx    string
}
...
func NewInfobloxProvider(cfg *StartupConfig, domainFilter *endpoint.DomainFilter) (*Provider, error) {
	...
	// AuthAwareRequestor suppresses the infoblox-go-client's automatic
	// grid-master repeat for requests that failed authentication or
	// authorization, so a misconfigured WAPI user is not retried.
	requestor := NewAuthAwareRequestor(&ibclient.WapiHttpRequestor{})
...
func (p *Provider) submitChanges(changes []*infobloxChange) error {
	...
	for zone, changes := range changesByZone {
		for _, change := range changes {
			...
			if actionErr != nil {
				// Issue #50: a rejected write used to be collected into errs and
				// aggregated with %v, which loses the HTTP status code and never
				// produced a log line at all when the aggregate was swallowed
				// upstream. Log it here, at the point of failure, so an
				// insufficiently privileged WAPI user is immediately visible.
				failure := ClassifyWAPIError(actionErr)
				objectType := record.obj.ObjectType()
				logWAPIFailure(objectType, change.Action, failure, 1, 1, actionErr)
				metrics.FailedApiCallsTotal.Inc()
				errs = append(errs, fmt.Errorf("could not %s %s record '%s' in zone '%s' (http status %d, %s): %w",
					strings.ToLower(change.Action), objectType, change.Endpoint.DNSName, zone,
					failure.StatusCode, failure.Kind, actionErr))
			}
		}
	}
...
func (p *Provider) ChangesByZone(zones []*ibclient.ZoneAuth, changeSets []*infobloxChange) map[string][]*infobloxChange {
	...
	for _, c := range changeSets {
		...
		if zone == nil || zone.Fqdn == "" {
			// Issue #50: this used to be a Debug line, so a requested change that
			// went nowhere left no trace at the default log level. An unmatched
			// record is either a misconfiguration or the symptom of a WAPI user
			// that cannot read the authoritative zones, so warn about it.
			log.WithFields(log.Fields{
				"record":            c.Endpoint.DNSName,
				"type":              c.Endpoint.RecordType,
				"action":            c.Action,
				"known_zones_count": len(zones),
			}).Warn("Skipping record because no authoritative Infoblox zone matching its DNS name was found. If the zone does exist, check the domain filter and whether the configured WAPI user is allowed to read zone_auth objects")
			continue
		}
...
// getObject performs a lookup for an existing record. Transient failures are
// retried, authentication and authorization failures are not, and every failure
// is logged with its HTTP status code and the WAPI object type.
func (p *Provider) getObject(obj ibclient.IBObject, queryParams *ibclient.QueryParams, res interface{}) error {
	return getObjectWithRetry(p.client, obj, "", queryParams, res, defaultGetAttempts)
}
...
func (p *Provider) recordSet(ep *endpoint.Endpoint, getObject bool) (recordSet infobloxRecordSet, err error) {
	...
	case endpoint.RecordTypeA:
		...
		if getObject {
			...
			err = p.getObject(obj, queryParams, &res)
			duration := time.Since(startTime)
			if err != nil && !isNotFoundError(err) {
	...
	case endpoint.RecordTypePTR:
		...
		if getObject {
			...
			duration := time.Since(startTime)
			if err != nil && !isNotFoundError(err) {
				err = fmt.Errorf("could not fetch PTR record ['%s':'%s'] : %w", *obj.PtrdName, *obj.Ipv4Addr, err)
				return
			}
	...
	case endpoint.RecordTypeCNAME:
		...
		if getObject {
			...
			duration := time.Since(startTime)
			if err != nil && !isNotFoundError(err) {
				err = fmt.Errorf("could not fetch CNAME record '%s': %w", *obj.Name, err)
				return
			}
	...
	case endpoint.RecordTypeNS:
		...
		if getObject {
			...
			duration := time.Since(startTime)
			if err != nil && !isNotFoundError(err) {
				err = fmt.Errorf("could not fetch NS record '%s': %w", obj.Name, err)
				return
			}
	...
	case endpoint.RecordTypeTXT:
		...
		if getObject {
			...
			duration := time.Since(startTime)
			if err != nil && !isNotFoundError(err) {
				err = fmt.Errorf("could not fetch TXT record '%s': %w", *obj.Name, err)
				return
			}
