...
//
// Two targets that are the same IP address in a different notation count as
// present, because "2001:db8::1" and "2001:0db8:0000:0000:0000:0000:0000:0001"
// address the same host. Targets that are not IP addresses are compared as
// plain strings.
func (rd ResponseDetails) Contains(target string) bool {
	want := canonicalTarget(target)
	for _, v := range rd {
		if canonicalTarget(v.Target) == want {
			return true
		}
	}
	return false
...
// canonicalTarget returns the canonical form of an IP address, so that the same
// address written in two notations yields the same string. A target that is not
